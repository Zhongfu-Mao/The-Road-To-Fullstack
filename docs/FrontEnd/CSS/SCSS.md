...
#### 1. [SCSS/SASS](https://www.sass.hk/docs/)
...
变量以美元符号开头（类似PowerShell），赋值方法与 CSS 属性的写法一样
...
* 数字：`1, 2, 13, 10px`
* 字符串，包括引号字符串与无引号字符串：`"foo", 'bar', baz`
...
~~~scss
// 总结：前部分为不带引号数字时，对比的仅仅是数字部分；反之，忽略引号，要求字符一一对应
~~~
...
~~~scss
// 总结：值与"and"、"or"和"not"之间必须要有空格，否则会被看做字符串
~~~
...
为了便于管理这样的属性，同时也为了避免了重复输入，Sass 允许将属性嵌套在命名空间中(类似with)
...
~~~scss
$family: unquote("Naruto+Hinata");
@import url("http://fonts.googleapis.com/css?family=\#{$family}");

// 编译为：
@import url("http://fonts.googleapis.com/css?family=Naruto+Hinata");
~~~
